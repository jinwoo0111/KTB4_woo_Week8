...
preparation_id="postgresql-pgtrgm-preparation-$(date '+%Y%m%d-%H%M%S-kst')"
result_dir="${PGTRGM_PREPARATION_RESULT_DIR:-${repository_root}/benchmark-data/postgresql/pgtrgm/${preparation_id}}"
...
docker compose --profile benchmark exec -T postgres-benchmark \
    psql -U community_benchmark -d "${experiment_database}" \
    -v ON_ERROR_STOP=1 -P pager=off \
    < "${sql_dir}/verify-pgtrgm-absent.sql" \
    > "${result_dir}/before-structure.log"

docker compose --profile benchmark exec -T postgres-benchmark \
    psql -U community_benchmark -d "${experiment_database}" \
    -v ON_ERROR_STOP=1 -P pager=off \
    < "${sql_dir}/capture-search-results.sql" \
    > "${result_dir}/search-results-before.txt"

docker compose --profile benchmark exec -T postgres-benchmark \
    psql -U community_benchmark -d "${experiment_database}" \
    -v ON_ERROR_STOP=1 -P pager=off \
    < "${sql_dir}/apply-pgtrgm.sql" \
    > "${result_dir}/apply-pgtrgm.log"
...
docker compose --profile benchmark exec -T postgres-benchmark \
    ...
    -v ON_ERROR_STOP=1 -P pager=off \
    < "${sql_dir}/capture-search-results.sql" \
    > "${result_dir}/search-results-after.txt"

cmp "${result_dir}/search-results-before.txt" \
    "${result_dir}/search-results-after.txt"
...
(
    ...
    shasum -a 256 \
        ...
        search-results-before.txt \
        apply-pgtrgm.log \
        after-structure.log \
        search-results-after.txt \
        canonical-verification-after.log \
        > SHA256SUMS
)

echo "pg_trgm experiment database is ready: ${experiment_database}"
