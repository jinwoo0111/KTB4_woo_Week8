...
measurement_id="postgresql-fts-${search_case}-$(date '+%Y%m%d-%H%M%S-kst')"
result_dir="${FTS_PERFORMANCE_RESULT_DIR:-${repository_root}/benchmark-data/postgresql/fts/performance/${measurement_id}}"
...
run_k6() {
    ...
    docker run --rm -i \
        -e BASE_URL="${base_url}" \
        -e KEYWORD="${keyword}" \
        -e SEARCH_CASE="fts_${search_case}_all" \
        -e K6_NO_COLOR=true \
...

{
    ...
    echo "fts_configuration=simple"
    echo "fts_query_function=plainto_tsquery"
    echo "ordering=post_id_desc"
...
echo "FTS ${search_case} performance measurement completed"
